...
export default function ThemeButton() {
  // Initialize to true since layout.tsx has className="light_theme" on html
  const [isLight, setIsLight] = useState(true);

  useEffect(() => {
    // Sync theme state from HTML element (in case it was changed elsewhere)
    const htmlElement = document.documentElement;
    const hasLightTheme = htmlElement.classList.contains("light_theme");
    setIsLight(hasLightTheme);
  }, []);
  ...
  return (
    <button
      onClick={handleClick}
      className={`${commonStyles.button} ${styles.theme_button}`}
